...
class Settings:
    ...
    UPLOADS_DIR: Path = Path(os.getenv("UPLOADS_DIR", PROJECT_ROOT / "uploads"))
    TEMPLATES_DIR: Path = Path(os.getenv("TEMPLATES_DIR", PROJECT_ROOT / "templates"))

    # 로깅 설정
    LOG_LEVEL: str = os.getenv("LOG_LEVEL", "INFO")
    LOG_FILE: Path = Path(os.getenv("LOG_FILE", PROJECT_ROOT / "logs" / "naverpost.log"))
    ...
    # 품질 검증 설정
    MIN_QUALITY_SCORE: float = float(os.getenv("MIN_QUALITY_SCORE", "70.0"))  # 최소 품질 점수
